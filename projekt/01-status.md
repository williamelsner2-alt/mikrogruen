...
| Datum | Was |
|---|---|
| 01.09.2026 | **Lautloser Zustellausfall — eine fehlende Uhrzeit, und niemand hat es gemerkt.** Unsere Nachricht vom 01.09. an `opt/besprechungsraum` lag stundenlang unabgeholt im eigenen Ausgang; der Postbote lief planmäßig und meldete jedes Mal „0 zugestellt, 0 im Ausgang, **keine Fehler**“. *Ursache, drüben im Skript gemessen:* Der Kopfzeile fehlte `HH:MM`, das Erkennungsmuster griff nicht — und **null erkannte Blöcke heißen in seiner Logik „leerer Ausgang“, nicht „unlesbarer Inhalt“**. Aufgefallen ist es nur, weil William die Gegenseite auf unsere Post ansprach. *Eigener Nachprüfbefund, der schlechtere:* Der kontoweite Skill `briefkasten` **schreibt die Uhrzeit vor** — es war keine Lücke im Sendeweg, sondern eine einzelne von Hand geschriebene Kopfzeile gegen die eigene Regel. Als **F-07** aufgenommen, mit der Gegenmaßnahme: Kopfzeile nach jedem Anhängen gegen das Muster prüfen — einmal angewandt, Prüfung bestanden, Antwort zugestellt. Die kaputte Nachricht bleibt **absichtlich stehen** als echter Negativtest für den dortigen Läufer-Umbau („Text ohne erkennbaren Block“ melden); unser Ausgang meldet dadurch dauerhaft Restinhalt — gewollt, kein neuer Fehler. *Sachantwort von drüben mitgebracht:* **Spitzenstufen-Nutzung hat kein eigenes Kontingent**, sie zählt aufs normale Wochenlimit und darf bis zu 50 % davon ausmachen — in `werkzeuge/arbeitsteilung.md` §2 nachgezogen, der Fable-Test erübrigt sich damit ersatzlos und ohne Verbrauch. `werkzeuge/kontingent.md` ist dort angekommen und übernommen, Audit-Vorschlag 26b angenommen (Rückmeldung steht aus). |
| 01.09.2026 | **Vier Entscheidungen Williams umgesetzt.** (1) **`werkzeuge/kontingent.md` freigegeben** und als Vorlage an `opt/besprechungsraum` geschickt — ausdrücklich als *Startpunkt ohne Abgleichpflicht*, mit einer Liste der darin enthaltenen Verweise, die nur bei uns gelten (unsere Pfade und Register-IDs zeigen dort ins Leere). Die Datei des Leitstands bleibt unberührt. (2) **Audit-Vorschlag 26b an claude-optimierung übergeben** — Chats umbenennen/löschen, **Projektgedächtnis korrigieren** (der belegt falsche uv-Eintrag steht dort weiter), Papierkorb leeren; passt zu 26a (Konto-Hygiene), das dort schon liegt. Der Papierkorb bleibt als Einrichtung, nur das Leeren ist Handarbeit. (3) **Der Fable-Test läuft dort, nicht hier** — meine Ankündigung vom selben Tag zurückgenommen, damit er nicht doppelt läuft; Ergebnis kommt zu uns zurück, es entscheidet über die Obergrenzen der Stufen-Spannen. (4) **Chat-Titel des Leitstands bestätigt:** `[LEIT] Leitstand`, eingetragen in `werkzeuge/instanzen-und-zugriffe.md` §3a — damit ist die Zuordnungstabelle für Mikrogrün **vollständig**. Audit-Vorschlag 6 (Cowork-Projekt) bleibt bewusst offen; er kommt voraussichtlich über claude-optimierung. |
| 01.09.2026 | **Live-Chat-Versuch mit dem Nachbarprojekt — und die Nutzungsanzeige ist vermessen.** *Versuch (Williams Aufbau):* zwei Dateien in `Claude\chat experiment\`, je ein Schreiber, keine Sperre nötig — sieben Runden in gut einer halben Stunde, **ohne dass William etwas weitergeben musste**. Drei Grenzen gemessen und in `werkzeuge/instanzen-und-zugriffe.md` §3 aufgenommen: Der Kanal trägt Gedanken, aber **keine Anlagen**; die Brücke zum Rechner **bricht nach 60 Sekunden ab** (getaktet, nicht flüssig); gleichzeitige Runden **kreuzen sich** (kein Datenverlust, aber doppelte Arbeit). *Sachergebnis:* Auf `claude.ai/settings/usage` stehen **drei getrennte Zähler als Text**, ohne Klickerei lesbar — der Browser-Weg aus I-24 ist damit **belegt statt benannt**, allerdings nur für Sitzungen mit verbundener Erweiterung; Cloud-Läufe bleiben blind. Grenzen der Anzeige (nur Prozent, Auflösung 1 %, Aktualisieren-Knopf) und die dabei gefundene **Rechenfalle** — Anker in Prozent, Verbrauch in Tokens, nicht verrechenbar; Auswege: Wechselkurs messen oder **Arbeitseinheiten statt Tokens zählen** — stehen in `werkzeuge/arbeitsteilung.md` §4. Register-Folgen (I-24, I-32) beim Leitstand angefragt. Kein Befund `4xx`, weil es die Umgebung betrifft und nicht das Vorhaben. |
| 01.09.2026 | **Postfach-Regel nachgezogen:** Erledigte Nachrichten werden **archiviert statt gelöscht** (Williams Entscheidung, im kontoweiten Skill `briefkasten` als Regel 2). In `werkzeuge/instanzen-und-zugriffe.md` §3 eingearbeitet. Außerdem dort neu: **Versionsangaben sind Momentaufnahmen** — im Nachbarprojekt hat sich die Claude-Code-CLI zwischen zwei Registereinträgen selbst aktualisiert; betrifft bei uns Node, Python und besonders den versionsnummerierten uv-Pfad des FreeCAD-MCP. |
| 25.08.2026 | **Richtigstellung zum Postboten-Befund (24.08.) und Schichtdienst-Anfrage abgelehnt.** *Richtigstellung:* Mein Verdacht auf ein **Verlustfenster** beim Postboten hat sich **nicht bestätigt** — claude-optimierung hat im Skript nachgesehen: Jeder Block wird zuerst am Ziel angehängt, die Quelle erst geleert, wenn alle Blöcke durch sind; ein Fehler dazwischen verhindert das Leeren, der nächste Lauf holt nach. Der Eintrag vom 24.08. bleibt stehen (nie löschen, nur markieren), die Warnung darin gilt nicht. Gefunden wurde beim Nachsehen ein kleineres Restrisiko (Doppelzustellung bei Abbruch, kein Verlust, dort als I-07); meine Punkte „Zustellungsnachweis statt Leeren" und „sichtbarer Betriebsstatus" sind dort als I-15 registriert. **Lehre:** Ein gemeldeter Verdacht ist eine Prüfbitte, kein Befund — die Prüfung gehört dem, der den Code hat. *Anfrage:* `opt/besprechungsraum` wollte unseren Schichtdienst mitbenutzen; abgelehnt mit Begründung — eine Cloud-Session hängt an **genau einem Projekt**, könnte also nur in **unsere** Ablage schreiben, und der Weg zu ihnen bräuchte zwingend eine laufende Mikrogrün-Session plus Postbote (nachts nicht vorhanden). Empfehlung an sie: lokal bauen, wo ihr Ordner zugleich ihre Ablage ist; dazu der Standby-Fallstrick aus Audit-Befund B9b. Die Warteschlange ist Leitstand-Lane — Anfrage dorthin weitergegeben, Entscheidung unberührt. |
